 ...
 * 免费版：每个项目可申请3个免费实例，7日内未使用即回收资源实例，仅支持加速22/3389端口，加速端口为后端随机分配。 
 
 * 基础版：适用于流量需求较小的日常运维场景，支持修改加速端口。
...
免费版globalssh将采用共享IP的方式分配资源，不同的实例会分配不同的端口用于SSH连接，不同于源站监听的SSH端口。
